...
The back-end application repository which contains the built static files for the SPA and the Docker Compose file for the entire application can be found on GitHub \cite{github_598975project-restapi_nodate}.
\newline
\newline
...
The code behind the built static front-end files can be found in the project front-end repository on GitHub \cite{github_598975project-frontend_2024}. W3CSS is used for styling the application \cite{W3CSS}.
\newline
\newline
...
The main portion of the messaging parts of the application can be found in the messaging repository on GitHub \cite{github_thorbjorn2021project-messaging_nodate}.
\newline
\newline
...
The broker server is active in the background checking for new messages. Whenever it detects a new message in the message queue, it receives this message, retrieves its contents and save them to the MongoDB database. Receiving the messages is handled by the Receiver.java class and saving the aggregated polls to the database is handler by the AggregatedPollRepository.java class.